...
async function fetchFiles(parentId: string, orderBy: string = 'createdTime') {
  ...
  const url = `${base_url}?key=${key}&q=${q}&fields=${fields}&orderBy=${orderBy}`
...
async function fetchContactAndConsultants(country: GoogleFile) {
  ...
  const consultantImageFiles = files.filter(
    f => f.mimeType === JPEG && f.name.toLowerCase() !== 'contact',
  )

  const consultantInfosFiles = files.filter(
    f => f.mimeType === XLS && f.name.toLowerCase() !== 'contact',
  )
